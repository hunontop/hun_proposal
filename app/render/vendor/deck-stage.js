...
(() => {
  ...
  const stylesheet = `
    ...
    /* Skipped slides stay legible (presenter previews backup/hidden content)
       — dim only slightly and tag with a small corner badge instead of a
       full-cover overlay. */
    .thumb[data-skip] .frame { opacity: 0.62; }
    ...
    .rail-resize {
      position: fixed;
      left: calc(var(--deck-rail-w, 230px) - 3px);
      top: 0;
      bottom: 0;
      width: 6px;
      cursor: col-resize;
  ...
  class DeckStage extends HTMLElement {
    ...
    _fit() {
      ...
      // Presenter notes dock reserves bottom space (--deck-reserve-bottom)
      // so the slide scales to sit fully ABOVE the notes instead of being
      // overlapped. Backward-compatible: unset/0 → original full-height fit.
      const rb = parseInt(getComputedStyle(this).getPropertyValue('--deck-reserve-bottom'), 10) || 0;
      if (stage) {
        stage.style.left = rw + 'px';
        // Shrink the stage's effective height and top-anchor it so the
        // centred canvas lands in the region above the reserved dock.
        stage.style.height = rb ? (window.innerHeight - rb) + 'px' : '';
        stage.style.bottom = rb ? 'auto' : '';
      }
