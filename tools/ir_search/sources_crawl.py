...
# [vendored patch — hun_proposal] tools/gongo_search.py가 서버측 검색 파라미터
# (condition/keyword 등)를 &key=value 형태로 주입한다. 빈 문자열이면 원본과 동일.
BIZINFO_EXTRA_QUERY = ""
...
def page_bizinfo(fetch, page):
    ...
    url = (
        BIZINFO_LIST_URL
        + f"?rows=15&cpage={page}&schEndAt=N" + BIZINFO_EXTRA_QUERY
    )
    status, h = fetch(url)
    if status != 200:
